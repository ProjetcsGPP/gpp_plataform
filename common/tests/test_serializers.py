...
from accounts.models import Aplicacao, Attribute, Role, User, UserRole
...
def setUpTestData(cls):
    ...
    Aplicacao.objects.filter(
        codigointerno__in=["ACOES_PNGI", "PORTAL", "CARGA_ORG_LOT"]
    ).delete()
    ...
    cls.user = User.objects.create_user(
        email="multiapp@example.com", name="Multi App User", password="testpass123"
    )
