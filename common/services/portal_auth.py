...
if TYPE_CHECKING:
    from accounts.models import Aplicacao, User
...
class PortalAuthService:
    ...
    def authenticate_user(self, token: str) -> Optional["User"]:
        ...
        try:
            ...
            if not email:
                logger.error(
                    f"[{self.app_code}] Email não encontrado nos dados do portal"
                )
                return None
            ...
            try:
                ...
                logger.warning(
                    f"[{self.app_code}] Usuário não encontrado localmente: {email}"
                )
                return None
    ...
    def _validate_token_with_portal(self, token: str) -> dict | None:
        """
        ...
        if not self.portal_url:
            ...
            logger.warning(
                f"[{self.app_code}] Modo desenvolvimento - validação mockada"
            )
            return {
    ...
    def sync_user(
        ...
        email: str,
        name: str,
        roles_data: list[str] | None = None,
        attributes_data: dict[str, str] | None = None,
    ...
    def _sync_user_roles(
        self, user: "User", app: "Aplicacao", role_codes: list[str]
    ) -> None:
        """
        ...
        logger.debug(
            f"[{self.app_code}] Aplicação: {app.codigointerno} (ID: {app.idaplicacao})"
        )
        logger.debug(f"[{self.app_code}] Roles solicitadas: {role_codes}")

        # Verificar quais roles existem no banco para esta aplicação
        existing_roles = Role.objects.filter(aplicacao=app).values_list(
            "codigoperfil", flat=True
        )
        logger.debug(
            f"[{self.app_code}] Roles disponíveis no banco: {list(existing_roles)}"
        )
        ...
        for role_code in role_codes:
            try:
                role = Role.objects.get(codigoperfil=role_code, aplicacao=app)
    ...
    def _sync_user_attributes(
        self, user: "User", app: "Aplicacao", attributes: dict[str, str]
    ) -> None:
        """
        ...
        for key, value in attributes.items():
            Attribute.objects.update_or_create(
                user=user, aplicacao=app, key=key, defaults={"value": str(value)}
            )
    ...
    def get_user_attributes(self, email: str) -> dict[str, str]:
        ...
        """
        from accounts.models import Attribute, User

        try:
            user = User.objects.get(email__iexact=email)
            app = self._get_application()

            attributes = Attribute.objects.filter(user=user, aplicacao=app)
