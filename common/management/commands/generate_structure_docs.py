...
class Command(BaseCommand):
    ...
    def get_apps_structure(self, verbose=False):
        ...
        structure = {"timestamp": datetime.now().isoformat(), "apps": {}}
    ...
    def analyze_app(self, app_config, app_path, verbose=False):
        ...
        if serializers_file.exists():
            app_data["serializers"] = self.extract_classes(
                serializers_file, "Serializer"
            )
        ...
        if permissions_file.exists():
            app_data["permissions"] = self.extract_classes(
                permissions_file, "Permission"
            )
    ...
    def write_markdown_docs(self, output_dir, structure_data):
        ...
        with open(md_file, "w", encoding="utf-8") as f:
            f.write("# 📦 Estrutura de Aplicacoes Django\n\n")
            ...
            for app_name, app_data in structure_data["apps"].items():
                ...
                if app_data.get("structure"):
                    f.write("### 📁 Estrutura de Diretorios\n\n")
                    f.write("```\n")
