...
from ..models import Attribute, User, UserRole
...
class TokenService:
    ...
    @staticmethod
    def _get_user_roles(
        user: User, aplicacao_code: str | None = None
    ) -> list[dict[str, Any]]:
    ...
    def _get_user_attributes(
        ...
        return [
            {
                "application__code": (
                    attr.aplicacao.codigointerno if attr.aplicacao else None
                ),
                "key": attr.key,
                "value": attr.value,
            }
