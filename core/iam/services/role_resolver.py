...
class RoleResolver:
    ...
    @staticmethod
    def get_user_roles_for_app(user: User, app_code: str) -> list[dict[str, Any]]:
        ...
        user_roles = UserRole.objects.filter(
            user=user, aplicacao__codigointerno=app_code
    ...
    @staticmethod
    def get_active_role(
        user: User, app_code: str, session_data: dict | None = None
    ) -> dict[str, Any] | None:
        ...
        if session_data and session_key in session_data:
            ...
            try:
                user_role = UserRole.objects.select_related("role", "aplicacao").get(
                    user=user, aplicacao__codigointerno=app_code, role_id=role_id
                )
                return {
    ...
    @staticmethod
    def set_active_role(
        user: User, app_code: str, role_id: int, session_data: dict
    ) -> None:
    ...
    @staticmethod
    def clear_active_role(app_code: str, session_data: dict) -> None:
    ...
    def get_highest_role(
        user: User, app_code: str, role_hierarchy: list[str]
    ) -> dict[str, Any] | None:
        ...
        for role_code in role_hierarchy:
            if role_code in user_role_codes:
                ...
                return next(role for role in user_roles if role["code"] == role_code)
