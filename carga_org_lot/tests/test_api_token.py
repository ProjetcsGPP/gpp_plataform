...
class TokenEnvioCargaViewSetTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="testuser_token@example.com", password="testpass123"
        )
        self.client.force_authenticate(user=self.user)

        self.status_progresso = TblStatusProgresso.objects.get_or_create(
            id_status_progresso=1, defaults={"str_descricao": "Em Progresso"}
        )[0]
...
class AuxiliaryViewSetsTest(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(
            email="testuser_aux@example.com", password="testpass123"
        )
        self.client.force_authenticate(user=self.user)
        ...
        TblStatusCarga.objects.get_or_create(
            id_status_carga=1, defaults={"str_descricao": "Iniciado", "flg_sucesso": 0}
        )
