    ...
    def setUpTestData(cls):
        ...
        try:
            ...
            cls.aplicacao, created = Aplicacao.objects.get_or_create(
                ...
                defaults={"nomeaplicacao": "Carga Org/Lot", "isshowinportal": True},
            )
            logger.info(
                f"Aplicação {'criada' if created else 'recuperada'}: {cls.aplicacao.codigointerno}"
            )
            ...
            logger.info(
                f"Role {'criada' if created else 'recuperada'}: {cls.role.codigoperfil}"
            )
            ...
            cls.user_role = UserRole.objects.create(
                user=cls.user, role=cls.role, aplicacao=cls.aplicacao
            )
            ...
            cls.status_progresso, created = TblStatusProgresso.objects.get_or_create(
                id_status_progresso=1, defaults={"str_descricao": "Em andamento"}
            )
            logger.info(
                f"Status Progresso {'criado' if created else 'recuperado'}: {cls.status_progresso.str_descricao}"
            )
    ...
    def setUp(self):
        ...
        try:
            ...
            if not self.client.session.get("_auth_user_id"):
                ...
                logger.error(
                    "❌ Falha ao configurar active_role_CARGA_ORG_LOT na sessão"
                )
            else:
                logger.info(
            ...
            logger.error(
                f"❌ ERRO NO setUp (force_login): {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
...
class AuthenticationViewsTest(BaseWebViewTestCase):
    ...
    def test_login_page_loads(self):
        ...
        try:
            ...
            logger.error(
                f"❌ ERRO em test_login_page_loads: {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
            raise
    ...
    def test_login_empty_fields(self):
        ...
        try:
            ...
            response = self.client.post(
                reverse("carga_org_lot_web:login"), {"email": "", "password": ""}
            )
            ...
            logger.error(
                f"❌ ERRO em test_login_empty_fields: {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
            raise
    ...
    def test_logout_redirects(self):
        ...
        try:
            ...
            logger.error(
                f"❌ ERRO em test_logout_redirects: {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
            raise
...
class DashboardViewTest(BaseWebViewTestCase):
    ...
    def test_dashboard_requires_login(self):
        ...
        try:
            ...
            logger.error(
                f"❌ ERRO em test_dashboard_requires_login: {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
            raise
    ...
    def test_dashboard_loads_for_authenticated_user(self):
        ...
        try:
            ...
            logger.error(
                f"❌ ERRO em test_dashboard_loads_for_authenticated_user: {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
            raise
...
class AjaxViewsTest(BaseWebViewTestCase):
    ...
    def setUpTestData(cls):
        ...
        try:
            ...
            logger.info(
                f"Organograma versão criado: {cls.organograma_versao.id_organograma_versao}"
            )
            ...
            logger.info(
                f"Órgãos criados para testes AJAX: {cls.orgao_raiz.str_sigla}, {cls.orgao_filho.str_sigla}"
            )

        except Exception as e:
            logger.error(
                f"❌ ERRO no setUpTestData de AjaxViewsTest: {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
            raise
    ...
    def test_search_orgao_ajax_returns_json(self):
        ...
        try:
            ...
            logger.error(
                f"❌ ERRO em test_search_orgao_ajax_returns_json: {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
            raise
    ...
    def test_search_orgao_ajax_short_query(self):
        ...
        try:
            ...
            logger.error(
                f"❌ ERRO em test_search_orgao_ajax_short_query: {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
            raise
...
class PaginationTest(BaseWebViewTestCase):
        ...
        try:
            ...
            for i in range(25):
                TblPatriarca.objects.create(
                    ...
                    id_externo_patriarca=str(
                        uuid.uuid4()
                    ),  # ✅ Gera UUID único, #f'EXT-ORG{i:02d}',  # CAMPO OBRIGATÓRIO
                    id_status_progresso=cls.status_progresso,
                    id_usuario_criacao=cls.user,
                )
            ...
            logger.error(
                f"❌ ERRO no setUpTestData de PaginationTest: {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
            raise
    ...
    def test_pagination_exists_with_many_records(self):
        ...
        try:
            ...
            logger.error(
                f"❌ ERRO em test_pagination_exists_with_many_records: {type(e).__name__}: {str(e)}"
            )
            logger.exception("Traceback completo:")
            raise
