...
def user_has_gestor_carga(user):
    ...
    return UserRole.objects.filter(
        user=user,
        aplicacao=aplicacao,
        role__codigoperfil='GESTOR_CARGA'
    ).exists()
    ...
    def _wrapped_view(request, *args, **kwargs):
        if not user_has_gestor_carga(request.user):
            return JsonResponse({
                ...
                'tabelas': ['tblstatusprogresso', 'tbltipocarga', 'tblstatustokenenviocarga', 'tblstatuscarga']
            }, status=403)
    ...
    def _wrapped_view(request, *args, **kwargs):
        if not user_has_coordenador_carga(request.user):
            return JsonResponse({
                ...
                'tabelas': ['tblpatriarca', 'tblorganogram*', 'tbllotacao*', 'tbltokenenviocarga', 'tblcargapatriarca*']
            }, status=403)
        return view_func(request, *args, **kwargs)
    return _wrapped_view
