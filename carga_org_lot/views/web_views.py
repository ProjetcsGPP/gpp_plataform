    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            try:
                ...
                login(request, user)
                messages.success(
                    request, f"Bem-vindo(a) ao Carga Org/Lot, {user.name}!"
                )
                return redirect("carga_org_lot_web:dashboard")
...
def organograma_hierarquia_json(request, organograma_id):
    ...
    """
    organograma = get_object_or_404(
        TblOrganogramaVersao, id_organograma_versao=organograma_id
    )
...
def lotacao_inconsistencias(request, lotacao_versao_id):
    ...
    """
    lotacao_versao = get_object_or_404(
        TblLotacaoVersao, id_lotacao_versao=lotacao_versao_id
    )
