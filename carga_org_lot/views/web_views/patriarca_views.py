...
def patriarca_detail(request, pk):
    ...
    organogramas = TblOrganogramaVersao.objects.filter(id_patriarca=patriarca).order_by(
        "-dat_processamento"
    )[:5]
    ...
    # Cargas recentes
    cargas = (
        TblCargaPatriarca.objects.filter(id_patriarca=patriarca)
...
def patriarca_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                status_nova_carga = TblStatusProgresso.objects.get(
            ...
            messages.error(
                request, "Erro ao criar patriarca. Verifique os campos abaixo."
            )
    else:
        form = PatriarcaForm()
...
def patriarca_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request, "Erro ao atualizar patriarca. Verifique os campos abaixo."
            )
    else:
...
def patriarca_reset(request, pk):
    ...
    try:
        ...
        messages.error(
            request, 'Status "Organograma em Progresso" (2) não encontrado no sistema.'
        )
        return redirect("carga_org_lot_web:patriarca_list")
