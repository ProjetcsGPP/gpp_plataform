...
class LotacaoJsonOrgaoViewSet(viewsets.ModelViewSet):
    ...
    def regenerar(self, request, pk=None):
        ...
        for lotacao in lotacoes:
            servidor = {
                ...
                "data_referencia": (
                    lotacao.dat_referencia.isoformat()
                    if lotacao.dat_referencia
                    else None
                ),
            }
    ...
    def enviar_api(self, request, pk=None):
        ...
        json_orgao.str_status_envio = (
            "PENDENTE"  # Deve ser atualizado após resposta da API
        )
    ...
    def estatisticas(self, request):
        ...
        stats = {
            ...
            "por_orgao": list(
                queryset.values(
                    "id_orgao_lotacao__str_sigla", "id_orgao_lotacao__str_nome"
                )
                .annotate(count=Count("id_lotacao_json_orgao"))
                .order_by("-count")[:10]
            ),
    ...
    def gerar_em_lote(self, request):
        ...
        request.data.get("sobrescrever", False)
