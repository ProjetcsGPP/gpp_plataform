...
class TokenEnvioCargaViewSet(viewsets.ModelViewSet):
    ...
    def finalizar(self, request, pk=None):
        ...
        if token.dat_data_hora_fim:
            return Response(
                {"error": "Token já foi finalizado"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"message": "Token finalizado com sucesso", "token": serializer.data}
        )
        ...
        return Response(
            {
                ...
                "duracao_minutos": (
                    self._calcular_duracao(token) if not valido else None
                ),
            }
        )
