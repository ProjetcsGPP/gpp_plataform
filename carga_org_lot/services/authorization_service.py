...
def get_user_permissions(user) -> dict:
    ...
    try:
        ...
        return {'roles': [], 'is_gestor': False, 'is_coordenador': False, 'is_operacional': False}
    ...
    def _wrapped_view(request, *args, **kwargs):
        ...
        if not permissoes['is_gestor']:
            return JsonResponse({
                ...
                'tabelas_afetadas': ['tblstatusprogresso', 'tbltipocarga', 'tblstatustokenenviocarga', 'tblstatuscarga']
            }, status=403)
    ...
    def _wrapped_view(request, *args, **kwargs):
        ...
        if not permissoes['is_operacional']:
            return JsonResponse({
                ...
                'roles_usuario': permissoes['roles'],
                'tabelas_afetadas': ['tblpatriarca', 'tblorganogram*', 'tbllotacao*', 'tblcargapatriarca*']
            }, status=403)
        return view_func(request, *args, **kwargs)
    return _wrapped_view
