...
class BasePermissionTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.user_no_role = User.objects.create_user(
            email="norole@seger.es.gov.br", name="User No Role", password="testpass123"
        )
...
class PermissionMatrixTests(BasePermissionTestCase):
    ...
    def test_permission_hierarchy_consistency(self):
        ...
        self.assertTrue(
            permissions_config_write["coordenador"]
            >= permissions_config_write["gestor"]
        )
        self.assertTrue(
            permissions_config_write["gestor"] >= permissions_config_write["operador"]
        )
        self.assertTrue(
            permissions_config_write["operador"]
            >= permissions_config_write["consultor"]
        )

        self.assertTrue(
            permissions_operacao_write["coordenador"]
            >= permissions_operacao_write["gestor"]
        )
        self.assertTrue(
            permissions_operacao_write["gestor"]
            >= permissions_operacao_write["operador"]
        )
        self.assertTrue(
            permissions_operacao_write["operador"]
            >= permissions_operacao_write["consultor"]
        )
