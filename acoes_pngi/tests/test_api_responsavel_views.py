...
from ..models import Acoes, RelacaoAcaoUsuarioResponsavel, UsuarioResponsavel
...
class UsuarioResponsavelViewSetTest(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.db_manager(db_alias).create_user(
            email="gestor@test.com", password="test123", name="Gestor Teste"
        )
        UserRole.objects.using(db_alias).create(
            user=self.user, aplicacao=self.app, role=self.role
        )
        ...
        self.user_resp1 = User.objects.db_manager(db_alias).create_user(
            email="resp1@test.com", password="test123", name="Responsável 1"
        )

        self.user_resp2 = User.objects.db_manager(db_alias).create_user(
            email="resp2@test.com", password="test123", name="Responsável 2"
        )

        self.user_resp3 = User.objects.db_manager(db_alias).create_user(
            email="resp3@test.com", password="test123", name="Responsável 3"
        )

        # Criar registros de usuários responsáveis
        self.responsavel1 = UsuarioResponsavel.objects.using(db_alias).create(
            idusuario=self.user_resp1, strtelefone="27999999999", strorgao="SEGER"
        )

        self.responsavel2 = UsuarioResponsavel.objects.using(db_alias).create(
            idusuario=self.user_resp2, strtelefone="27988888888", strorgao="SEDU"
        )
        ...
        assert (
            UsuarioResponsavel.objects.using(db_alias)
            .filter(idusuario=self.user_resp1)
            .exists()
        ), "Dados não visíveis na DB da API!"

        print(
            f"✅ Teste dados: {UsuarioResponsavel.objects.using(db_alias).count()} registros"
        )
    ...
    def test_list_responsaveis_requires_authentication(self):
        ...
        print(
            f"Results[0] completo: {response.data['results'][0] if 'results' in response.data and response.data['results'] else 'Vazio'}"
        )
        print("=== FIM DEBUG ===")
    ...
    def test_create_responsavel(self):
        ...
        response = self.client.post(
            "/api/v1/acoes_pngi/usuarios-responsaveis/", data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(
            UsuarioResponsavel.objects.filter(idusuario=self.user_resp3).exists()
        )
    ...
    def test_delete_responsavel(self):
        ...
        self.assertFalse(
            UsuarioResponsavel.objects.filter(idusuario=responsavel_id).exists()
        )
    ...
    def test_search_responsaveis_by_email(self):
        ...
        print(
            f"DEBUG: User resp1 existe: {User.objects.filter(email='resp1@test.com').exists()}"
        )
        print(
            f"DEBUG: Responsavel resp1 existe: {UsuarioResponsavel.objects.filter(idusuario__email='resp1@test.com').exists()}"
        )
    ...
    def test_ordering_responsaveis_by_name(self):
        ...
        if results:
            ...
            if "idusuario_name" in sample:
                ...
                isinstance(sample.get("idusuario"), dict)
                and "name" in sample["idusuario"]
            ):
...
class RelacaoAcaoUsuarioResponsavelViewSetTest(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.relacao1 = RelacaoAcaoUsuarioResponsavel.objects.create(
            idacao=self.acao1, idusuarioresponsavel=self.responsavel1
        )

        self.relacao2 = RelacaoAcaoUsuarioResponsavel.objects.create(
            idacao=self.acao1, idusuarioresponsavel=self.responsavel2
        )
    ...
    def test_retrieve_relacao(self):
        ...
        self.assertEqual(
            response.data["idusuarioresponsavel"], self.responsavel1.idusuario.id
        )
    ...
    def test_create_relacao(self):
        ...
        response = self.client.post(
            "/api/v1/acoes_pngi/relacoes-acao-responsavel/", data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(
            RelacaoAcaoUsuarioResponsavel.objects.filter(
                idacao=self.acao2, idusuarioresponsavel=self.responsavel1
            ).exists()
        )
