...
class TipoEntraveAlertaAPITests(BaseAPITestCase):
    ...
    def test_coordenador_cannot_create_tipo_entrave(self):
        ...
        response = self.client.post(
            "/api/v1/acoes_pngi/tipos-entrave-alerta/", data, format="json"
        )
...
class AcoesAPITests(BaseAPITestCase):
    ...
    def test_coordenador_can_update_acao(self):
        ...
        response = self.client.patch(
            f"/api/v1/acoes_pngi/acoes/{self.acao.idacao}/", data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class AcaoPrazoAPITests(BaseAPITestCase):
    ...
    def test_coordenador_can_update_prazo(self):
        ...
        response = self.client.patch(
            f"/api/v1/acoes_pngi/prazos/{self.prazo.idacaoprazo}/", data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_coordenador_can_delete_prazo(self):
        ...
        prazo_temp = AcaoPrazo.objects.create(
            idacao=self.acao, strprazo="2026-12-31", isacaoprazoativo=False
        )

        self.authenticate_as("coordenador")
        response = self.client.delete(
            f"/api/v1/acoes_pngi/prazos/{prazo_temp.idacaoprazo}/"
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
    ...
    def test_list_only_active_prazos(self):
        ...
        AcaoPrazo.objects.create(
            idacao=self.acao, strprazo="2026-03-31", isacaoprazoativo=False
        )
    ...
    def test_filter_prazos_by_acao(self):
        ...
        self.authenticate_as("consultor")
        response = self.client.get(
            f"/api/v1/acoes_pngi/prazos/?idacao={self.acao.idacao}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertGreater(len(response.data), 0)
...
class AcaoDestaqueAPITests(BaseAPITestCase):
    ...
    def setup_test_data(self):
        ...
        self.destaque = AcaoDestaque.objects.create(
            idacao=self.acao, datdatadestaque=timezone.now()
        )
    ...
    def test_coordenador_can_create_destaque(self):
        ...
        response = self.client.post(
            "/api/v1/acoes_pngi/destaques/", data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_filter_destaques_by_acao(self):
        ...
        self.authenticate_as("consultor")
        response = self.client.get(
            f"/api/v1/acoes_pngi/destaques/?idacao={self.acao.idacao}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertGreater(len(response.data), 0)
    ...
    def test_destaques_ordered_by_ordenacao(self):
        ...
        AcaoDestaque.objects.create(idacao=self.acao, datdatadestaque=timezone.now())
        AcaoDestaque.objects.create(idacao=self.acao, datdatadestaque=timezone.now())

        self.authenticate_as("consultor")
        response = self.client.get(
            "/api/v1/acoes_pngi/destaques/?ordering=-datdatadestaque"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertGreater(len(response.data), 0)
