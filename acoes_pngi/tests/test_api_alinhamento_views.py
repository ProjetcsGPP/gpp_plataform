...
class AcaoAnotacaoAlinhamentoViewSetTest(APITestCase):
    ...
    def test_anotacao_with_all_relationships(self):
        ...
        self.assertEqual(
            response.data["idacaoanotacaoalinhamento"],
            self.anotacao1.idacaoanotacaoalinhamento,
        )
    ...
    def test_partial_update_anotacao(self):
        ...
        data = {"strdescricaoanotacaoalinhamento": "Descrição atualizada parcialmente"}
    ...
    def test_ordering_anotacoes(self):
        ...
        response = self.client.get(
            f"{self.api_base_url}?ordering=-datdataanotacaoalinhamento"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_search_anotacoes_by_descricao(self):
        ...
        self.assertTrue(
            any(
                "Descrição" in r.get("strdescricaoanotacaoalinhamento", "")
                for r in results
            )
        )  # Algum resultado tem "Descrição"
    ...
    def test_update_anotacao(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["strdescricaoanotacaoalinhamento"], "Descrição atualizada"
        )
