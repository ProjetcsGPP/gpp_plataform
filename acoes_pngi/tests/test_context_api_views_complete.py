...
class BaseContextAPITestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="gestor@seger.es.gov.br", name="Gestor Test", password="testpass123"
        )
        ...
        self.user_no_access = User.objects.create_user(
            email="noaccess@test.com", name="No Access", password="testpass123"
        )
...
class ContextAPIIntegrationTests(BaseContextAPITestCase):
    ...
    def test_context_data_consistency_across_endpoints(self):
        ...
        if full_response.data["app"]["code"]:
            self.assertEqual(
                app_response.data["code"], full_response.data["app"]["code"]
            )

        if full_response.data["permissions"]["user_id"]:
            self.assertEqual(
                perms_response.data["user_id"],
                full_response.data["permissions"]["user_id"],
            )
    ...
    def test_multiple_users_get_correct_contexts(self):
        ...
        user2 = User.objects.create_user(
            email="operador@test.com", name="Operador Test", password="test123"
        )

        UserRole.objects.create(user=user2, aplicacao=self.app, role=role2)
