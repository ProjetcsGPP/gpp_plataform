...
from common.serializers import BaseModelSerializer, TimestampedModelSerializer
...
class EixoSerializer(TimestampedModelSerializer):
    ...
    class Meta:
        model = Eixo
        fields = ["ideixo", "strdescricaoeixo", "stralias", "created_at", "updated_at"]
    ...
    def validate_stralias(self, value):
        ...
        if not value.isupper():
            raise serializers.ValidationError(
                "O alias deve estar em letras maiúsculas."
            )
        if len(value) > 5:
            raise serializers.ValidationError(
                "O alias deve ter no máximo 5 caracteres."
            )
        return value
    ...
    def validate_strdescricaoeixo(self, value):
        ...
        if not value or not value.strip():
            raise serializers.ValidationError(
                "A descrição do eixo não pode estar vazia."
            )
        return value.strip()
...
class SituacaoAcaoSerializer(BaseModelSerializer):
    ...
    class Meta:
        model = SituacaoAcao
        fields = ["idsituacaoacao", "strdescricaosituacao", "created_at", "updated_at"]
    ...
    def validate_strdescricaosituacao(self, value):
        ...
        if not value or not value.strip():
            raise serializers.ValidationError(
                "A descrição da situação não pode estar vazia."
            )
...
class VigenciaPNGISerializer(TimestampedModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        """
        if (
            validated_data.get("isvigenciaativa", False)
            and not instance.isvigenciaativa
        ):
            with transaction.atomic():
                VigenciaPNGI.objects.filter(isvigenciaativa=True).update(
                    isvigenciaativa=False
...
class UsuarioResponsavelSerializer(TimestampedModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        try:
            ...
            # Debug para testes
            if hasattr(self.context.get("request"), "testing") or self.context.get(
                "debug", False
            ):
...
class RelacaoAcaoUsuarioResponsavelSerializer(TimestampedModelSerializer):
    ...
    idusuarioresponsavel_display = serializers.CharField(
        source="idusuarioresponsavel.idusuario.name", read_only=True
    )
...
class AcoesSerializer(TimestampedModelSerializer):
    ...
    idvigenciapngi_display = serializers.CharField(
        source="idvigenciapngi.strdescricaovigenciapngi", read_only=True
    )
...
class AcoesListSerializer(BaseModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "strdescricaoentrega",
            "idvigenciapngi",
            "idvigenciapngi_display",
            "idtipoentravealerta",
            "idtipoentravealerta_display",
            "datdataentrega",
            "created_at",
            "updated_at",
        ]
...
class AcoesCompletasSerializer(serializers.ModelSerializer):
        ...
        return (
            {
                ...
                "strdescricaosituacao": (
                    obj.idsituacaoacao.strdescricaosituacao
                    if obj.idsituacaoacao
                    else None
                ),
            }
            if obj.idsituacaoacao
    ...
    def get_responsaveis_completos(self, obj):
        ...
        if hasattr(obj, "responsaveis_completos"):
            return UsuarioResponsavelCompletoSerializer(
                obj.responsaveis_completos, many=True, context=self.context
            ).data
        return []
