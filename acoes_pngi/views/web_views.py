    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            try:
                ...
                user_role = (
                    UserRole.objects.filter(user=user, aplicacao=app_acoes)
                    .select_related("role")
                    .first()
                )
...
def acoes_pngi_dashboard(request):
    ...
    if "view_vigenciapngi" in permissions:
        ...
        stats["vigencias_ativas"] = VigenciaPNGI.objects.filter(
    ...
    if request.method == "POST":
        ...
        try:
            eixo = Eixo.objects.create(
                strdescricaoeixo=strdescricaoeixo, stralias=stralias
            )
            messages.success(
                request, f'Eixo "{eixo.strdescricaoeixo}" criado com sucesso!'
            )
...
def eixo_update(request, pk):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            eixo.save()
            messages.success(
                request, f'Eixo "{eixo.strdescricaoeixo}" atualizado com sucesso!'
            )
            return redirect("acoes_pngi_web:eixos_list")
        except Exception as e:
...
def vigencias_list(request):
    ...
    return render(
        request,
        "acoes_pngi/vigencias/list.html",
        {
            "vigencias": vigencias,
        },
    )
...
def vigencia_create(request):
    ...
    if request.method == "POST":
        ...
        # Validações
        if not all(
            [anovigencia, descricaovigencia, datavigenciainicio, datavigenciafim]
        ):
            messages.error(request, "Todos os campos são obrigatórios.")
        ...
        try:
            ...
            if fim <= inicio:
                messages.error(
                    request, "A data de fim deve ser posterior à data de início."
                )
                return render(
                    request,
                    "acoes_pngi/vigencias/form.html",
                    {
        ...
        try:
            ...
            messages.success(
                request, f"Vigência {vigencia.anovigencia} criada com sucesso!"
            )
...
def vigencia_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if not all(
            ...
            messages.error(request, "Todos os campos são obrigatórios.")
            return render(
                request, "acoes_pngi/vigencias/form.html", {"vigencia": vigencia}
            )
        ...
        if isvigenciaativa and not vigencia.isvigenciaativa:
            VigenciaPNGI.objects.filter(isvigenciaativa=True).exclude(pk=pk).update(
                isvigenciaativa=False
            )
        ...
        try:
            ...
            messages.success(
                request, f"Vigência {vigencia.anovigencia} atualizada com sucesso!"
            )
            return redirect("acoes_pngi_web:vigencias_list")
        except Exception as e:
