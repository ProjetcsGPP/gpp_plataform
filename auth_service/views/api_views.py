...
from accounts.models import Attribute, User, UserRole
    ...
    def get_token(cls, user):
        ...
        roles = list(
            UserRole.objects.filter(user=user).values(
                "aplicacao__codigointerno", "role__codigoperfil", "role__nomeperfil"
            )
        )
        token["roles"] = roles
        ...
        attrs = list(
            Attribute.objects.filter(user=user).values(
                "aplicacao__codigointerno", "key", "value"
            )
        )
        token["attrs"] = attrs
...
def session_login(request):
    ...
    try:
        ...
        if not email or not password:
            return JsonResponse(
                {"ok": False, "message": "Email e senha são obrigatórios."}, status=400
            )
        ...
        if user is not None:
            ...
            for ur in user_roles:
                ...
                apps_dict[app_code]["roles"].append(
                    {"codigo": ur.role.codigoperfil, "nome": ur.role.nomeperfil}
                )
        ...
        return JsonResponse(
            {"ok": False, "message": f"Erro no servidor: {str(e)}"}, status=500
        )
