...
def load_domain_data(apps, schema_editor):
    """
    Migração CONSOLIDADA ACCOUNTS - APENAS DADOS DE DOMÍNIO
    ...
    aplicacoes_data = [
        {
            'idaplicacao': 1, 'codigointerno': 'PORTAL',
            'nomeaplicacao': 'Portal GPP', 'base_url': 'http://127.0.0.1:8000/portal/',
            'isshowinportal': False
        },
        {
            'idaplicacao': 2, 'codigointerno': 'CARGA_ORG_LOT',
            'nomeaplicacao': 'Carga Única de Organograma e Lotação',
            'base_url': 'http://127.0.0.1:8000/carga_org_lot/', 'isshowinportal': True
        },
        {
            'idaplicacao': 3, 'codigointerno': 'ACOES_PNGI',
            'nomeaplicacao': 'Ações PNGI', 'base_url': 'http://127.0.0.1:8000/acoes-pngi/',
            'isshowinportal': True
        }
    ]
    ...
    print("✅ Accounts domain data carregado (sem Groups Django)")

def reverse_migration(apps, schema_editor):
    """Rollback parcial preservando dados estruturais"""
    pass
