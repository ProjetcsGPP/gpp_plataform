...
# 📋 MATRIZ DE PERMISSÕES CORRIGIDA (baseada nos dados reais)
MATRIZ_PERMISSOES = {
    # ============================================================================
    # GESTOR_PNGI (id=3) - Acesso total
    # ============================================================================
    'GESTOR_PNGI': {
        # Nível 1 - Configurações Críticas (somente GESTOR pode W/D)
    ...
    # ============================================================================
    # COORDENADOR_PNGI (id=5) - Nível 2 + Operações
    # ============================================================================
    'COORDENADOR_PNGI': {
        ...
        # Nível 2 - Escrita permitida
        'eixo': ['view', 'add', 'change', 'delete'],
        'vigenciapngi': ['view', 'add', 'change', 'delete'],
        'tipoanotacaoalinhamento': ['view', 'add', 'change', 'delete'],
        
        # Operações - Escrita permitida
        'acoes': ['view', 'add', 'change', 'delete'],
        'acaoprazo': ['view', 'add', 'change', 'delete'],
        'acaodestaque': ['view', 'add', 'change', 'delete'],
        'acaoanotacaoalinhamento': ['view', 'add', 'change', 'delete'],
    ...
    # ============================================================================
    # OPERADOR_ACAO (id=6) - Apenas Operações
    # ============================================================================
    'OPERADOR_ACAO': {
...
def popular_permissoes(apps, schema_editor):
    ...
    print("\n" + "="*80)
    print("🔐 POPULANDO PERMISSÕES - ACOES_PNGI")
    print("="*80 + "\n")
                    ...
                    for acao in acoes:
                        ...
                        try:
                            ...
                            if not group.permissions.filter(id=permission.id).exists():
                                ...
                                print(f"   ℹ️  Já existe: {codename}")
                                
                        except Permission.DoesNotExist:
                            erros += 1
                            print(f"   ❌ Permission não encontrada: {codename}")
                
                except ContentType.DoesNotExist:
                    erros += 1
                    print(f"   ⚠️  ContentType não encontrado: {app_label}.{modelo_name}")
        
        except Group.DoesNotExist:
            print(f"   ⚠️  Grupo não encontrado: {group_name}")
    
    print("\n" + "="*80)
    print("📊 RESUMO FINAL")
    print("="*80)
    print(f"✅ Permissões adicionadas: {permissoes_adicionadas}")
    print(f"❌ Erros: {erros}")
    print(f"📈 Total processado: {total_permissoes}")
    print("\n🎉 MIGRATION CONCLUÍDA COM SUCESSO!")
    
    # Verificação final
    gestor_group = Group.objects.get(name=f"{app_code}_GESTOR_PNGI")
    print(f"\n✅ GESTOR_PNGI tem {gestor_group.permissions.count()} permissões")
    print(f"   Amostra: {list(gestor_group.permissions.values_list('codename', flat=True))[:5]}...")


def reverter_permissoes(apps, schema_editor):
    """Remove permissões dos grupos PNGI."""
    Aplicacao = apps.get_model('accounts', 'Aplicacao')
    ...
    for role_code in MATRIZ_PERMISSOES.keys():
        ...
        try:
            ...
            print(f"🗑️  Permissões removidas: {group_name}")
        except Group.DoesNotExist:
            pass
