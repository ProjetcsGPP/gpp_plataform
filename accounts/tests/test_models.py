...
from ..models import Aplicacao, Attribute, Role, UserRole
...
class UserModelTest(TestCase):
    ...
    def test_email_unique(self):
        ...
        User.objects.create_user(email="test@example.com", password="pass1")
...
class AplicacaoModelTest(TestCase):
    ...
    def test_codigo_unique(self):
        ...
        Aplicacao.objects.create(codigointerno="TEST", nomeaplicacao="App 1")
        with self.assertRaises(IntegrityError):
            Aplicacao.objects.create(codigointerno="TEST", nomeaplicacao="App 2")

    def test_aplicacao_str(self):
        """Testa representação string"""
        app = Aplicacao.objects.create(
            codigointerno="MY_APP", nomeaplicacao="Minha Aplicação"
        )
...
class RoleModelTest(TestCase):
    ...
    def setUp(self):
        self.app = Aplicacao.objects.create(
            codigointerno="TEST_APP", nomeaplicacao="Test"
        )

    def test_create_role(self):
        """Testa criação de role"""
        role = Role.objects.create(
            aplicacao=self.app, nomeperfil="Administrador", codigoperfil="ADMIN"
        )
    ...
    def test_role_unique_per_app(self):
        ...
        Role.objects.create(
            aplicacao=self.app, nomeperfil="Admin", codigoperfil="ADMIN"
        )
        with self.assertRaises(IntegrityError):
            Role.objects.create(
                aplicacao=self.app, nomeperfil="Administrador", codigoperfil="ADMIN"
...
class UserRoleModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.role = Role.objects.create(
            aplicacao=self.app, nomeperfil="Gestor", codigoperfil="GESTOR"
        )
...
class AttributeModelTest(TestCase):
    ...
    def test_create_attribute(self):
        ...
        attr = Attribute.objects.create(
            user=self.user, aplicacao=self.app, key="can_upload", value="true"
        )
    ...
    def test_attribute_unique(self):
        ...
        with self.assertRaises(IntegrityError):
            Attribute.objects.create(
                user=self.user, aplicacao=self.app, key="can_upload", value="false"
            )
