    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            user, created = User.objects.get_or_create(
                username=cpf, defaults={"first_name": serializer.validated_data["nome"]}
            )

            # Role específica da app
            role = Role.objects.get(
                codigoperfil=role_codigo, aplicacao__codigointerno=app_codigo
            )

            # Atribuir role
            UserRole.objects.get_or_create(
                user=user, role=role, aplicacao=role.aplicacao
            )
...
class ChangeRoleView(APIView):
    ...
    def post(self, request):
        ...
        app_code = request.data.get(
            "app_code", token_payload.get("app_code", "ACOES_PNGI")
        )

        if not role_id:
            return Response(
                {"detail": "role_id é obrigatório"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Valida se o usuário tem essa role na aplicação
        try:
            user_role = UserRole.objects.select_related("role", "aplicacao").get(
                user=request.user, role_id=role_id, aplicacao__codigointerno=app_code
            )
