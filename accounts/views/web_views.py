...
class WebLoginView(TemplateView):
    ...
    def post(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        try:
            ...
            token_data = token_service.login(
                username,
                password,
                app_code,
                int(role_id or 1),  # ✅ Dinâmico!
            )
    ...
    def _get_validation_context(self, request: HttpRequest) -> dict[str, Any]:
        ...
        context = {
            ...
            "token_payload_json": json.dumps(
                getattr(request, "token_payload", {}), indent=2, default=str
            ),
            # Active role
            ...
            "session_data": (
                dict(request.session.items()) if request.session.session_key else {}
            ),
            ...
            # Roles disponíveis
            "available_roles": (
                getattr(request.token_payload, "roles", [])
...
class WebUserManagementView(TemplateView):
    ...
    def post(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        try:
            ...
            if action == "toggle_active":
                ...
                if success:
                    messages.success(
                        request, f"Senha resetada para {user_id}: {new_pass}"
                    )
                else:
    ...
    def _get_users_context(self, request: HttpRequest) -> dict[str, Any]:
        ...
        context = {
            ...
            "can_manage": request.token_payload.get("roles", []).filter(
                lambda r: r in ["GESTOR_PNGI", "ADMIN"]
            ),
        ...
        logger.info(
            f"Gestão usuários web - {context['total_users']} usuários carregados"
        )
        return context
